...
# 1 Install Calico for k8s and flannel 
### calico 3.3 Install (基于Flannel网络 并且etcd集群使用tls验证)
kubeadm 搭建的单主机k8s请参考：
...
data:
  ...
  etcd_endpoints: "https://127.0.0.1:2379" #etcd_endpoints 需要写入k8s集群etcd地址
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: calico-kube-controllers
          image: quay.io/calico/kube-controllers:v3.3.0
          env:
            # The location of the Calico etcd cluster.
            - name: ETCD_ENDPOINTS
              valueFrom:
